...
%	Returns: meta- updated meta data struct
    ...
    %undersc = strfind(meta.fpath,'_');           % get date of file
    ...
    meta.results_folder = strcat('../Results/seizure-detection/',meta.date,...
		'/',meta.experiment_name,'/');
	meta.fname = cur_file.name;
    meta.fpath = [ meta.dname filesep meta.fname ];

	meta.log_fpath = strcat(meta.results_folder,strrep(cur_file.name,'.smr','.log'));
	meta.log_fpath = strcat(meta.results_folder,strrep(cur_file.name,'.smrx','.log'));
